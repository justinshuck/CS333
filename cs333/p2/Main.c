...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  function ProducerConsumer ()
      ...
      -- The mutexLock, fullCounter with '0' and the emptyCounter with the BufferSize
      fullCounter.Init(0)
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    ...
    methods
      ...
      areNeigborsEating(p: int) returns bool -- Checks to see if the left & right neighbors states are EATING
  ...
  endMethod
    ...
    method PutDownForks (p: int)
      -- This method is called when the philosopher 'p' is done eating.
        ...
        self.changeState(self.getRight(p))  -- Change states of right/left neighbors
    ...
    method areNeigborsEating(p: int) returns bool
      -- This is used to check the left/right neighbrs status and see if they're eating
      return ((status[self.getRight(p)] != EATING) && (status[self.getLeft(p)] != EATING))
    endMethod
    ...
    method changeState(p: int)
      ...
      if (self.areNeigborsEating(p) && status[p] == HUNGRY)
        status[p] = EATING
        self.PrintAllStatus()     -- A change has occured so I need to re-print all statuses
        cond[p].Signal(&mLock)
      endIf
    endMethod
