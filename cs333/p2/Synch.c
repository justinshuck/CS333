  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Lock ()
          ...
          --Check to see if the thread we're trying to lock is already locked
          if currentThread == heldBy
             FatalError("Error: The lock is already being held")
          endIf
      ...
      method Unlock ()
          var 
            oldIntStat: int
            nextThread: ptr to Thread = null


          oldIntStat = SetInterruptsTo (DISABLED)

          --Ensures we're not unlocking a thread we're not holding in heldBy
          if heldBy != currentThread
             FatalError("Error: Attempting to unlock a thread you don't own")
          endIf
